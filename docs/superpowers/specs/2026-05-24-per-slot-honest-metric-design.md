...
| 7 | Public API contract on the headline field | **Add new field, repoint old gradually.** Add `perSlotCensorshipPct` to `/api/v1/summary` immediately. For ~30 days the existing `censorshipPct` keeps its per-payload semantics; `/api-docs` flags the switch date. On switch day, `censorshipPct` value (not field name) changes to per-slot; `perSlotCensorshipPct` stays as an explicit-name alias. |
